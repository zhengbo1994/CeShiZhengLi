...
$(function () {
    ...
    var controllerName = "PrintCertificate";
    var $printCertificate_QueryArea = $('#divPrintCertificate_QueryArea');
    var $gridPrintCertificate = $("#gridPrintCertificate_main");
    var $pagerPrintCertificate = $("#pagerPrintCertificate_main");
    var $mdlPrintCertificateInfo = $("#mdlPrintCertificate_CertificateInfo");

    //初始化查询区域
    var initQueryArea = function () {
        var initQueryButton = function () {
            $("#btnPrintCertificate_Query").on("click", function () {
                var queryData = {};
    ...
    var initPrintCertificateGrid = function () {
        ...
        $gridPrintCertificate.jqGrid({
            ...
            datatype: "json",
            postData: queryData,
            colNames: ["EmployeeId", "姓名", "身份证号", "性别", "证书编号", "证书有效期", "发证日期", "出生年月", "企业名称", "职务", "技术职称", "考试计划流水号", "考核点名称", "证书行业", "证书类别", "打印状态", "操作"],
            colModel: [
                    ...
                    { name: "ExamPlanNumber", index: "ExamPlanNumber", align: "center", width: 150 },
                    { name: "TrainingInstutionName", index: "TrainingInstutionName", align: "center", width: 200 },
                    { name: "Industry", index: "Industry", align: "center", width: 80 },
                    { name: "ExamType", index: "ExamType", align: "center", width: 80 },
                    { name: "PrintStatus", index: "PrintStatus", align: "center", width: 80 },
                    {
                        name: "操作", index: "操作", key: true, width: 150, align: "center", formatter: function (cellvalue, options, rowobj) {

                            var buttons = ''
                              + '<a href="#" title="打印证书" onclick="PrintCertificate(' + rowobj.EmployeeId + ')" style="padding: 7px;line-height: 1em;">'
    ...
    var initButtonArea = function () {
        var getCertificateData = function (employeeId) {
            var resultData = {};
            var ajaxOpt = {
        ...
        var getPhoto = function (employeeId) {
            var resultData = "";
            var ajaxOpt = {
        ...
        var loadCertificateData = function (employeeId) {
            var certificateData = getCertificateData(employeeId);
            //登记照
            ...
            var photoUrl = getPhoto(employeeId);
        ...
        var printCertificate = function (employeeId) {
            loadCertificateData(employeeId);
            $mdlPrintCertificateInfo.find("div[name='divCertificatePrint']").jqprint({ operaSupport: true });
            savePrintCertificateRecord();
        };
        window.PrintCertificate = printCertificate;
        $("#btnPrintCertificate_Print").on("click", function () {
            var rowId = $gridPrintCertificate.jqGrid("getGridParam", "selrow");
            if (!rowId) {
                alert("请选择要打印的证书记录");
                return false;
            }
            var employeeId = $gridPrintCertificate.jqGrid("getRowData", rowId).EmployeeId;
            loadCertificateData(employeeId);
            $("#mdlPrintCertificate_CertificateInfo").modal("toggle");
        });
