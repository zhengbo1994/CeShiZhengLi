...
$(function () {
    ...
    var controllerName = "CertificateEnterpriseChange";
    var $divMainQueryArea = $('#divCertificateEnterpriseChange_QueryArea');
    var $gridMain = $("#gridCertificateEntChang_main");
    var $pagerMain = $("#pagerCertificateEntChang_main");
    var $divMainInfo = $("#divCertificateEntChange_CertificateEntChangeInfo");
    var $mdlMainInfo = $("#mdlCertificateEntChange_CertificateEntChangeInfo");
    var enum_EditTypes = {
        insert: 0,
        update: 1,
        view: 2
    ...
    var getSelectedRowDataOfGrid = function (rowid) {
        var selRowId = "";
        if (!rowid) {
            $pagerMain.jqGrid("getGridParam", "selrow");
        }
        selRowId = rowid;
        var rowData = {};
        if (selRowId && null != selRowId) {
            rowData = $pagerMain.jqGrid("getRowData", selRowId);
        }
        else {
            abortThread("请选中行！");
        }
    ...
    var initMainGrid = function () {
        ...
        $gridMain.jqGrid({
            ...
            multiboxonly: true,
            postData: queryData,
            colNames: ["Id", "持证人姓名", "性别", "出生年月", "身份证号", "联系方式", "企业名称", "职务", "技术职称", "证书编号", "证书类别", "证书状态", "操作"],
            colModel: [
    ...
    var initButtonArea = function () {
        ...
        //初始化插入按钮
        var initCertificateEntChange_EntChange = function () {
            $("#btnCertificateEntChange_EntChange").on("click", function () {
                currentEdittype = enum_EditTypes.insert;
                $mdlMainInfo.modal("toggle");
                // showCertificateManagementModel();
            });
        }
        initCertificateEntChange_EntChange();
    }
