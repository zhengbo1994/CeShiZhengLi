/**
 * 注意：容器id（render方法传递的参数）是：Div+调用构造函数FusionCharts传递的参数id
 * 即：如果new FusionCharts（'/IDWebSite/Common/Flash/MSCombiDY2D.swf','Chart123'），则render('DivChart123')，容器ID为DivChart123
...
infosoftglobal.FusionCharts.prototype = {
    ...
    //将百分比转换成实际宽度 陈毓孟 2012-12-07
    getPixelWidth: function (width, containerID)
    {
        //将百分比转换成实际宽度
        if (typeof (width) == "string")
        {
            ...
            if (index != -1)
            {
                ...
                if (containerDiv)
                {
                    width = (containerDiv.offsetWidth * parseFloat(width.substring(0, index)) / 100).toString();
                }
            }
        }
        return width;
