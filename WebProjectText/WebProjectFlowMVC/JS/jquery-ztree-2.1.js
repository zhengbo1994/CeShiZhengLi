...
(function($) {
	...
	$.fn.zTree = function(zTreeSetting, zTreeNodes) {
		...
		var setting = {
			...
			//radio 最大个数限制类型，每一级节点限�?�?整棵Tree的全部节点限制（checkStyle=Check_Style_Box时无效）
			checkRadioType:Radio_Type_Level,
			//checkRadioType = Radio_Type_All 时，保存被选择节点的堆�?
	...
	function showTree(setting, treeNode) {
		...
		function(eventMouseDown) {
			...
			$(doc).mousemove(function(event) {
				...
				//避免鼠标误操作，对于第一次移动小于MinMoveSize时，不开启拖拽功�?
...
//			return false;
	...
	function addEditBtn(setting, treeNode) {		
		...
		if (right < 0) {
			//如果节点处于tree的最右侧，为避免无法正常操作按钮，则在左侧显�?
			editBtnObj.remove();
	...
	//设置CheckBox的Class类型，主要用于显示子节点是否全部被选择的样�?
	...
	function switchNode(setting, treeNode) {
		...
		//.net下刷新页面的bug可能还是button造成的，因此将一下代码暂时屏�?
...
//        return false;
	...
	//获取全部 checked = true or false 的节点集�?
	...
	function zTreePlugin(){
		return {
			...
			expandNode : function(treeNode, expandSign, sonSign) {
				...
				if (sonSign) {
					//多个图层同时进行动画，导致产生的延迟很难用代码准确捕获动画最终结束时�?
					//因此为了保证准确将节点focus进行定位，则对于js操作节点时，不进行动�?
