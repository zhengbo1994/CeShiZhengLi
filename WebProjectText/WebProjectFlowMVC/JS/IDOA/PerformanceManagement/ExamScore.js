...
function reloadData()
{
    ...
    $.post('FillData.ashx', { action: 'GetExamStationScoreTable', EPSID: sEPSID, AccountID: sAccountID, CorpID: strCorpID, ExamID: strExamID }, function (data, textStatus) { loadListData(data) });
    ...
    {
        $.post('FillData.ashx', { action: 'GetExamScopeScoreDetail', EPSID: sEPSID }, function (data, textStatus) { loadDetailData(data) });
...
var loadListData = function (data)
{
    ...
    {
        getObj("dlist").innerHTML = "当前没有符合条件的已报批岗位绩效！";
    }
}
...
function ddlCorp_change(vCTID, vCID)
{
    ...
    $.post('FillData.ashx', { action: 'GetExamPlanByCorpID', CorpID: sCropID, AccountID: sAccountID,
        ExamFilter: "2", IsResultApproval: "Y", IsNoSelect: 'N'
...
function ddlExamPlan_change(vCTID, vCID)
{
    ...
    $.post('FillData.ashx', { action: 'GetExamScopeOrgByExamID', ExamID: sExamID, AccountID: sAccountID, ScopeType: "S", IsTeamLeader: '', IsResultApproval: 'Y', IsNoSelect: 'N' }, function (data, textStatus)
    {
        loadExamScope(data, vCTID + '|' + vCID); ddlExamScope_change();
    }, 'json');
