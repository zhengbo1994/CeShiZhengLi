...
{
    openAddWindow("VMyPerformanceAssessBookAdd.aspx", 0, 0, "jqPerformanceAssessBook");
}


//修改
function showMyPerformanceAssessBookEdit(PABID)
{
    $("#jqPerformanceAssessBook").resetSelection();
    $("#jqPerformanceAssessBook").setSelection(PABID, false);
    var url = "VMyPerformanceAssessBookEdit.aspx?PABID=" + PABID;
    openModifyWindow(url, 0, 0, "jqPerformanceAssessBook");
}

//查看
function showMyPerformanceAssessBookBrowse(PABID)
{
    openWindow("VPerformanceAssessBookBrowse.aspx?ID=" + PABID, 0, 0);
}

//删除
function btnDelete_Click()
{
    openDeleteWindow("PerformanceAssessBook", 1, "jqPerformanceAssessBook");
}


//撤销
function revokeMyPerformanceAssessBook()
{
    openRevokeWindow("PerformanceAssessBook", "jqPerformanceAssessBook");
}
...
function reloadData()
{
    var jqObj = $('#jqPerformanceAssessBook', document);
    ...
    refreshJQGrid('jqPerformanceAssessBook');
}
...
function addPerformanceBill()
{
    ...
    var ExamPlanScopeInfo = openModalWindow('../../../Common/Select/IDOA/VSelectMultiExamPlanScope.aspx?ExamID=' + ExamID + '&tbID=' + tbPerformanceBill.id, 900, 600);
...
function showPerformanceBillDetail(EPSID, StruName)
{
    ...
    return '<a href="#" onclick="openWindow(\'' + url + '\',0, 0)">' + StruName + '</a>';
}
...
function validateSize()
{
    ...
    if (getObj("hidExamID").value == "")
    {
        return alertMsg("考核计划不能为空。", getObj("txtExamName"));
    }
    ...
    else
    {
        ...
        {
            return alertMsg("直接登记时考核内部编号不能为空。", getObj("txtOldPABNo"));
        }
    }
    ...
    if (IDs.length == 0)
    {
        return alertMsg("考核目标明细不能为空。", tbPerformanceBill);
    }    
...
/* 考核目标浏览页方法 */
function showKPIIndexList()
{
    showPerformanceBillDetailList('KPI');
}

function showBehaviorIndexList()
{
    showPerformanceBillDetailList('Behavior');
}
function showExamPlanProgressList()
{
    showPerformanceBillDetailList('Progress');
}
