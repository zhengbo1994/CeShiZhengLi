...
var MyAlert = (function ()
{
    ...
    myAlert.prototype._setPostionParent = function ()
    {
        ...
        if (/relative/.test(this.contentBoxPos.toLowerCase()))
        {
            this.$contentBox.css("position", "static");
        }
    }
    ...
    myAlert.prototype._bindEvent = function ()
    {
        ...
        this.$btnsWrap.bind("mouseover", function (e)
        {
            
            var $target = $(e.target);
            if ($target.hasClass("btn-default") && !$target.hasClass("btn-default-active"))
            {
                $target.addClass("btn-default-active");
            }
        });

        this.$btnsWrap.bind("mouseout", function (e)
        {
            var $target = $(e.target);
            if ($target.hasClass("btn-default") && $target.hasClass("btn-default-active"))
            {
                $target.removeClass("btn-default-active");
            }
        });
        ...
        this.$titleWrap.bind("mouseover", function (e)
        {
            var $target = $(e.target);
            if ($target.hasClass("close") && !$target.hasClass("close-active"))
            {
                $target.addClass("close-active");
            }
        });

        this.$titleWrap.bind("mouseout", function (e)
        {
            var $target = $(e.target);
            if ($target.hasClass("close") && $target.hasClass("close-active"))
            {
                $target.removeClass("close-active");
            }
        });
    }
    ...
    myAlert.prototype._afreshRender = function ()
    {
        ...
        this.$message.html(currentConfig["message"])
            .removeClass().addClass("alert-message icon-" + (typeID[currentConfig["id"]] || "reading") + "-" + (iconSize[currentConfig["iconSize"]] || "l"))
