...
var Page = (function (win) {
    ...
    */
    page.prototype.init = function (silverPageInstance) {
        ...
        //顶部菜单图标文字提示
        var contentBox=this.$topBody.find('.frame-content-box');
    	this.$funcbar.children('a').each(function () {
    		$(this).mouseover(function (evt) {
    			var linkItemMsg=$('<div class="link-item-Msg"></div>');
      			if (!$(this).attr('urlname')) return;
    			linkItemMsg.html($(this).attr('urlname'));
    			linkItemMsg.css({left:evt.clientX+10,top:evt.clientY+10})
    			contentBox.append(linkItemMsg);
    			$(this).mousemove(function (event) {
    				linkItemMsg.css({left:event.clientX+10,top:event.clientY+10})
    			})
   ...
   */
    remindWorks.prototype.setWaitWork = function (from, strNewWork) {
        ...
        if (from === 0) {
            ...
            this.remindWorks.Height = (this.remindWorks.Count > 0 ? (this.remindWorks.Height > 500 ? 500 : this.remindWorks.Height) : 90);
            ...
            if (this.remindWorks.Count > 999) {
                this.$remindElement.find("i").html("999+");
            } else {
                this.$remindElement.find("i").html((this.remindWorks.Count > 0 ? this.remindWorks.Count : ""));
            }
            //设置点击弹出窗口的内容
...
*/
    remindWorks.prototype.analyseWorkRemind = function (iModuleDefIndex, strNewWork, strOldWork, bShowWaitDo, bShowWaitRead) {
        ...
        if (work.Count > 0) {
            ...
            this.remindWorks.Height += ((lineCnt + 1) * 36); //将每行都改为36px  邓洪涛 2017-08-07
        }
    }
    ...
    */
    remindWorks.prototype.setWorkRemindHtml = function () {
        ...
        html.push('<table class="idtbfix index_msg_border" style="height:100%;"><tr><td class="index_msg_header">'
            ...
            , '<td><div style=\'padding-left:18px\'>', this.remindDefine.Caption
            , (this.remindWorks.PressDoCount && this.remindWorks.PressDoCount != "0" ? ("<span style='font-size:13px;color:#0b80ff;cursor:hand' onclick='parent.openWindow(\"../Common/Personal/VWaitAllWork.aspx?WaitDo=1&IsPressedDo=Y\",960, 650)'> <span style='color:#414141'>--</span> 催办<span style='color:#414141'>[</span><span style='color:#0b80ff;font-weight:normal;'>" + this.remindWorks.PressDoCount + "</span><span style='color:#414141'>]</span></span>") : "")
	 ...
	 */
        ...
        for (var i = 0; i < arrWork.length; i++) {
            ...
            stringFormat('<td style="width:{0}%" nowrap><a onclick="{1}" class="font hand" title="{2}"{5}>{3}[<span style="color:red">{4}</span>]</a>{6}</td>',
                ...
                ' style="text-decoration:none" onmouseover="this.style.color=\'#0b80ff\'" onmouseout="this.style.color=\'\'"',
    ...
    remindWorks.prototype.registerPopup = function (arrNewWorks) {
        ...
        if (arrNewWorks && arrNewWorks.length > 0) {
            ...
            pop.closeCallbacks.push(function (me) {
                if (me.closeByBtn) {//手动点击关闭时，才停止闪烁标题                
...
$(function () {
    ...
    var showORCode = function (downMenu, menu) {
        if (downMenu.css("display") === "none") {
            ...
            var offWidth = window.top.document.body.clientWidth - winWidth;
            if (offWidth != 0) {
                x = x + offWidth - 150;
            }
