...
var SilverPage = (function () {
    ...
    fn.bootstrapper = function () {
        ...
        this.duckSilder.binding(function () {
            if ($('.duck-menu-silder').css('display') == 'block') {
                that.menuSilder.collapse();
            }
        });

        //菜单收起时点击按钮展开菜单
        $(".another-duck-silder").click(function () {
        ...
        this.vm.load(function (vm) {
            ...
            win.on("resize", function () {
                ...
                // 窗口改变后表格区域自适应
				$('#frame-content').css('height',SilverPage.$win.height()-36)
            }).trigger("resize");
...
SilverPage.ShortcutMenu = (function () {
    ...
    fn.show = function (x, y) {
        ...
        if ($('.layout-drag-splitter').offset().left == 0) {
            x = x || offset.left;
        } else {
            x = (x + $('.duck-menu-silder').width()) || offset.left + $('.duck-menu-silder').width();
        }
        y = y || offset.top + parseInt(this.target.css("font-size")) + 20;

        x = (x + this.menus.width()) >= winWidth - 5 ? winWidth - this.menus.width() - 25 : x - 36;
...
SilverPage.MenuSilder = (function () {
    ...
    function menuSilder(widthCallback) {
        ...
        this.$searchbox = this.$obj.find('.search-menu-box');
        ...
        this.oldWidth = 258;    // 上一次菜单栏被调整的宽度（用于在重新展开时计算尺寸）
    ...
    fn.searchMenu = (function () {
        ...
        searchmenu.prototype.init = function (vm) {
            ...
            this.$input.on("focus", function () {
                $('.search-input-box').removeClass('input-blur-class').addClass('input-focus-class');
                $('.placeholder-words').hide();
                if (that.slideTimer) {
            ...
            .on("blur", function () {
                $('.search-input-box').removeClass('input-focus-class').addClass('input-blur-class');
                if (!$(this).val()) {
                    $('.placeholder-words').show();
                }
    ...
    fn.init = function () {
        ...
        //点击<首页>按钮
        this.$searchbox.on("click", ".system-menu.go-main-page", function () {
            var url = "../" + $(this).attr("data-main-target");
            if (url) {
        ...
        this.$menus.on("click", ".sub-menu > ul > li", function () {
            ...
            if (me.hasClass("has-third-menu")) {
                ...
                //若没有下一级则让其高亮显示
                $('.main-menu').find('li').removeClass('selected-color');
    ...
    fn.buildMainMenuContent = function (vm) {
        ...
        $(menus).each(function (i, e) {
            menuname = e["name"] || "";
			if (e["ico"]) {
				e["ico"]=e["ico"].replace(/TenderBlue/ig,'CloudsWhite');
			}
            result.push(
...
SilverPage.ContentFrame = (function () {
    function contentFrame() {
		...
		this.$obj.css('height',SilverPage.$win.height()-36)
    ...
    fn.fill = function (vm) {
        if (vm.Data.HomePage) {
            vm.Data.HomePage = "Home/portal/portal1.html";
...
SilverPage.TopFrame = (function () {
    ...
    fn.tryfill = function () {
        ...
        if (this.vm === null) {
            ...
            //console.log(vm.Data.LogoLinks)

            var format = '<a href="{url}" data-url-id="{urlid}" class="normal-menu{hasChildren}"{target}>{urlname}{dropDownArrow}</a>{children}',
                phoneFormat = '<a href="{url}" urlname="{urlname}" data-url-id="{urlid}" class="normal-menu{hasChildren} phoneType"{target}></a>{children}',
                startWork = '<a href="{url}" urlname="{urlname}" data-url-id="{urlid}" class="normal-menu{hasChildren} workType"{target}></a>{children}',
                createCommunicate = '<a href="{url}" urlname="{urlname}" data-url-id="{urlid}" class="normal-menu{hasChildren} communicateType"{target}></a>{children}',
                createKnowlege = '<a href="{url}" urlname="{urlname}" data-url-id="{urlid}" class="normal-menu{hasChildren} KnowlegeType"{target}></a>{children}',
                setting = '<a href="{url}" urlname="{urlname}" data-url-id="{urlid}" class="normal-menu{hasChildren} setting"{target}></a>{children}',
                share = '<a href="{url}" urlname="{urlname}" data-url-id="{urlid}" class="normal-menu{hasChildren} share"{target}></a>{children}',
                icoFormat = '<img src="..{ico}" alt="" /> ';
				
            // 指定 LOGO 图片
            if (vm.Data.LogoImage === "Image/Logo/eap-logo.png") {
                $(".logo-area").find('img').attr("src", "../Image/Logo/eap-color-logo.png")
            } else {
                $(".logo-area").find('img').attr("src", "../" + vm.Data.LogoImage)
            };
            ...
            //}
            var homeButton = SilverPage.$body.find(".search-menu-box").find(".go-main-page");
            if (!homeButton.length) {
                SilverPage.$body.find(".search-menu-box").append('<a href="javascript: void 0" data-main-target="' + vm.Data.HomePage + '" class="system-menu go-main-page"></a>')
            }

            that.$funcbar.append('<a href="javascript: void 0" class="system-menu show-wait-working" urlname="待办事项"><i></i></a>');
            //that.$funcbar.find('.atag-wrapper').append('<a href="javascript: void 0" class="system-menu go-main-page" data-main-target="Home/portal/portal.html"></a>');
            //计算适合的宽度
            ...
            if (vm.Data.LogoLinks && vm.Data.LogoLinks.length > 0) {
                $(vm.Data.LogoLinks).each(function (i, elem) {
					...
					switch (elem["urlname"]){
						case "手机版":
						formatString=phoneFormat;
							break;
						case "发起工作":
						formatString=startWork;
							break;
						case "新建沟通":
						formatString=createCommunicate;
							break;
						case "新建知识":
						formatString=createKnowlege;
							break;
						case "设置":
                    ...
                    that.$funcbar.append(
                              formatString.replace(/\{url\}/g, url || "javascript: void 0")
    ...
    fn.fillUserInfo = function (vm) {
        ...
        //      var userContainer = $("#topping").find('.user-area');
