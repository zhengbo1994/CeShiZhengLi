    ...
    fn.bootstrapper = function ()
    {
        ...
        this.topFrame.init(
                            ...
                            function (top)    // 当顶部框架页面加载完毕后执行的回调
                            {
                                ...
                                }).on("click", "a.normal-menu.has-children", function ()      // 当点击顶部菜单并改菜单存在下来菜单时，显示子菜单
                                {
                                    var me = $(this),
                                ...
                                }).on("mouseenter", "a.normal-menu.has-children", function ()      // 当子菜单有一个显示时，鼠标移到顶部菜单时，显示子菜单
                                {
        ...
        this.vm.load(function (vm)
        {
            ...
            win.on("resize", function ()
            {
                ...
                if (that.menuSilder.hasMenu() && win.width() < 810)
                {
...
SilverPage.ShortcutMenu = (function ()
{
    ...
    fn.show = function (x, y)
    {
        ...
        x = x || offset.left;
        y = y || offset.top + parseInt(this.target.css("font-size")) + 10;

        x = (x + this.menus.width()) > winWidth ? winWidth - this.target.width() : x;
...
SilverPage.MenuSilder = (function ()
{
    ...
    function menuSilder(widthCallback)
    {
        ...
        this.$operat = this.$obj.find(".menu-bar-operat");
        ...
        this.$user = this.$obj.find(".user-info");

        this.widthCallback = widthCallback;

        this.minWidth = 30;     // 菜单最小宽度，小于此宽度则自动隐藏
    ...
    fn.init = function ()
    {
        ...
        //将 li.has-other-menu 改为 li  -----change by denght--20170717
    ...
    fn.showSubMenu = function (mainmenu, finished)
    {
        ...
        if (submenu.length > 0)
        {
            ...
            submenu.show().siblings(".sub-menu").hide();
        ...
        if (subMenu.length > 0)
        {
            if (subMenu.css("display") === "none")
            {
                ...
                subMenu.slideDown(disableAnimate ? 0 : 220, function ()
                {
                    ...
                    if (p < 1)
                    {
                        var m = subMenu.closest(".sub-menu").children("ul");
    ...
    fn.fillUserInfo = function (vm)
    {
        ...
        if (vm.Data.IsIdeaAdmin === 'Y' && this.$refresh.length > 0)
        {
            this.$refresh.show();
        } else
        {
        ...
        this.$user.find(".profile-setting img").on("error", function ()
        {
        ...
        }).on("load", function ()
        {
            ...
            /**#endregion by zm**/
        }).attr("src", "../" + u.HeadImg + "?x=" + setTimeout(" ", 0));


        this.$user.find(".user-name").text(u.EmployeeName || "<未知用户>");
    ...
    fn.buildMainMenuContent = function (vm)
    {
        ...
        $(menus).each(function (i, e)
        {
            ...
            result.push(
                    (menuname === "设置中心" ? settingFormat : format)
                          .replace(/\{ico\}/g, e["ico"] || "")
    ...
    {
        var menus = this.vm.Data.MenuMod,
            format = '<li data-id="{id}" class="{class}" data-ico="{ico}" data-linkway="{linkway}"{target}>{name}</li>',
            target = ' target="{url}"',
            that = this;
...
SilverPage.TopFrame = (function ()
{
    ...
    fn.tryfill = function ()
    {
        ...
        } else
        {
            ...
            that.$funcbar.append('<a href="javascript: void 0" class="system-menu show-wait-working">待办 <i>(0)</i></a>');
            ...
            if (vm.Data.LogoLinks && vm.Data.LogoLinks.length > 0)
            {
                $(vm.Data.LogoLinks).each(function (i, elem)
                {
                    ...
                    that.$funcbar.append(
                                format.replace(/\{url\}/g, url || "javascript: void 0")
