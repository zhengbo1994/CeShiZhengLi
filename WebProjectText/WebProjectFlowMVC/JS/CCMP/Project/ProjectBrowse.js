...
function windowLoad()
{
    ...
    if(getObj("hidCauseCostIsShape").value=="Y")
    {
        getObj("trShapeDate").style.display="";
        getObj("trVersionNo").style.display="";
        getObj("trCauseCost").style.display="";
    }
    
    getObj("hrObject").style.display =  getObj("hidSubjectIsShare").value == "Y"?"none":"";
    getObj("hrSubjectContract").style.display=getObj("hidCauseCostShapeMothod").value=="1"?"":"none";
...
//                    +'&RealID='+realID+"&RealOption="+realOption;
    ...
    var url = '/' + rootUrl + "/CCMP/ContractPlan/ProjectContractPlan/VProjectContractPlanView.aspx?SourceType=ProjectBrowse&ProjectID=" + projectID;
