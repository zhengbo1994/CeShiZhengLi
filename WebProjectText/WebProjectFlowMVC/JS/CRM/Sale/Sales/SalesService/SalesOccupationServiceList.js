//changcx
//time:2013-5-22 15:55:51
var request = new QueryString();
var proj = request["pID"];
var tj = request["tj"];
//加载
function reloadData() {
    var jqObj = $('#jqOccupationList', document);
    ...
    addParamsForJQGridQuery("jqOccupationList", [query]);
    refreshJQGrid("jqOccupationList");
}
//搜索
...
function showSalesOrderService(rowid, iRow, iCol, e) {
    var saleOrderId = $('#jqOccupationList').getRowData(rowid)["SalesOrderGUID"];
    var SalesServiceDetailGUID = $('#jqOccupationList').getRowData(rowid)["SalesServiceDetailGUID"];
    var url = "SalesOccupation.aspx?SalesOrderServiceID=" + rowid + "&SaleOrderGUID=" + saleOrderId + "&SalesServiceDetailGUID=" + SalesServiceDetailGUID + "&projID=" + $("#ddlProject").val() + "";
    // alert(url);
...
function showDeal(serviceType) {
    var IDs = getJQGridSelectedRowsID("jqOccupationList", true);
...
function getDataForDeal(urlstr, param) {
    ...
    if (param == "SaleOrderID") {
        IDs = getJQGridSelectedRowsData("jqOccupationList", true, "SalesOrderGUID");
    }
    if (param == "SalesServiceDetailGUID") {
        IDs = getJQGridSelectedRowsData("jqOccupationList", true, "SalesServiceDetailGUID");
    }
    url = urlstr + "?" + param + "={0}";
    url = stringFormat(url, IDs.join(','));
    return url;
...
function addSalesServiceDetail() {
    ...
    var url = "SalesDealService.aspx?SalesServiceDetailGUID=" + SalesServiceDetailGUID + "&SalesOrderServiceGUID=" + SalesOrderServiceGUID + "&type=d&serviceType=4&projID=" + $("#hidProjectID").val();
    openAddWindow(url, 500, 450);
