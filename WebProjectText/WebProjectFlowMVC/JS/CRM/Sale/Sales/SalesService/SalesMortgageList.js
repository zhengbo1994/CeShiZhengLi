...
//加载
function reloadData() {
    var jqObj = $('#jqmortgageList', document);
    ...
    addParamsForJQGridQuery("jqmortgageList", [query]);
    refreshJQGrid("jqmortgageList");
}
//搜索
...
function showSalesOrderService(rowid, iRow, iCol, e) {
    var saleOrderId = $('#jqmortgageList').getRowData(rowid)["SalesOrderGUID"];
    var SalesServiceDetailGUID = $('#jqmortgageList').getRowData(rowid)["SalesServiceDetailGUID"];
    var url = "SalesMortgage.aspx?SalesOrderServiceID=" + rowid + "&SaleOrderGUID=" + saleOrderId + "&SalesServiceDetailGUID=" + SalesServiceDetailGUID + "&projID=" + $("#ddlProject").val() + "";
    // alert(url);
...
function showBankList() {
    var IDs = getJQGridSelectedRowsID("jqmortgageList", true);
    if (null == IDs || IDs.length == 0) {
        alert('请选择记录！'); return;
    }
    var url = getDataForDeal("SalesMortgageBankList.aspx", "SaleOrderID");
...
function getDataForDeal(urlstr, param) {
    ...
    if (param == "SaleOrderID") {
        IDs = getJQGridSelectedRowsData("jqmortgageList", true, "SalesOrderGUID");
    }
    if (param == "SalesServiceDetailGUID") {
        IDs = getJQGridSelectedRowsData("jqmortgageList", true, "SalesServiceDetailGUID");
    }
    url = urlstr + "?" + param + "={0}";
    url = stringFormat(url, IDs.join(','));
    return url;
...
function addSalesServiceDetail() {
    ...
    var url = "SalesDealService.aspx?SalesServiceDetailGUID=" + SalesServiceDetailGUID + "&SalesOrderServiceGUID=" + SalesOrderServiceGUID + "&type=d&serviceType=3&projID=" + $("#hidProjectID").val();
    openAddWindow(url, 500, 450);
...
function Check() {
    ...
    var lendMoney = $("#txtLendingBankMoney").val();
    ...
    if (bank == "") {
        alert('请录入‘按揭银行’！'); return false;
    }
    if (parseInt(lendMoney) > parseInt(contactMoney)) {
        alert('请确保按揭贷款额度小于合同总价！'); return false;
    }
    if (lendMoney != lendMoneyBefore) {
        var bValue = confirm('按揭金额已修改，是否重新生成付款详情？');
