...
{
    var url = "'../../../IDOA/Consume/Maintenance/VMaintenanceBrowse.aspx?ID=" + rowobject[0] + "'";
...
function btnChoose_Click()
{
    ...
    var vCPRID = getJQGridSelectedRowsID('jqMaintenance', true);
    var vCCNo = getJQGridSelectedRowsData('jqMaintenance', true, 'CCNO');
    var vCCTitle = getJQGridSelectedRowsData('jqMaintenance', true, 'CCTitle');
    ...
    if (vAction == "Reimbursement")
    {
        ...
        for (var i = 0; i < vCPRID.length; i++)
        {
            ...
            cell.align = "center";
            cell.innerHTML = '<input id="chkIDV3" style="width:15px;height:15px;" type="checkbox" value="' + vCPRID[i] + '" modeltype="Maintenace" onclick="selectRow(this);" />';
            ...
            cell.innerHTML = '<a href="#showRelateName" onclick="openWindow(\'/IDOA/Consume/Maintenance/VMaintenanceBrowse.aspx?ID=' + vCPRID[i] + '\',0,0)">' + $.jgrid.stripHtml(vCCTitle[i]) + '</a>';
    ...
    else
    {
        ...
        for (var i = 0; i < vCPRID.length; i++)
        {
            ...
            var row = dgRelate.insertRow(); // 合同关联用品维护

            var cell = row.insertCell(0);
            cell.align = "center";
            cell.innerHTML = '<input style="width:15px;height:15px;" type="checkbox" onclick="selectRow(this)"  gisttype="6" value="' + vCPRID[i] + '" />';
            ...
            cell = row.insertCell(3);
            cell.innerHTML = '<a href="#showRelateName" onclick="showRelateName(\'' + vCPRID[i] + '\',\'6\')">' + $.jgrid.stripHtml(vCCTitle[i]) + '</a>';
