...
{
    /// <summary>
    ///     添加了接口中对于CommandTimeout属性的实现
    ///     在所有增删改查方法中添加了对CommandTimeout参数的设置
    /// </summary>
    public class MySqlDatabase : IDatabase
    {
        ...
        public int Delete<T>(object propertyValue, string propertyName) where T : class
        {
            ...
            IEnumerable<T> entitys = dbTransaction.Connection.Query<T>(string.Format("select * from {0} where {1}=?propertyValue", EntityAttribute.GetEntityTable<T>(), propertyName), new { propertyValue = propertyValue }, null, true, CommandTimeout);
        ...
        public IEnumerable<T> FindList<T>() where T : class,new()
        {
            using (var dbConnection = Connection)
            {
                return dbConnection.Query<T>(string.Format("SELECT * FROM {0} ", EntityAttribute.GetEntityTable<T>()), null, null, true, CommandTimeout).ToList();
            }
        }
